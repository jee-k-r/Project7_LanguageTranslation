...
assert LooseVersion(tf.__version__) in [LooseVersion('1.13.1'), LooseVersion('1.13.1')], 'This project requires TensorFlow version 1.0  You are using {}'.format(tf.__version__)
...
def encoding_layer( rnn_inputs, rnn_size, num_layers, keep_prob):
    ...
    #drop = tf.compat.v1.nn.rnn_cell.DropoutWrapper(lstm, keep_prob)
    def get_a_cell(rnn_size, keep_prob):
            lstm = tf.nn.rnn_cell.BasicLSTMCell(rnn_size)
            drop = tf.nn.rnn_cell.DropoutWrapper(lstm, output_keep_prob=keep_prob)
            return drop
    #enc_cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell([drop] * num_layers)
    with tf.name_scope('lstm'):
         enc_cell = tf.nn.rnn_cell.MultiRNNCell([get_a_cell(rnn_size, keep_prob) for _ in range(num_layers)]  )
    _, enc_state = tf.compat.v1.nn.dynamic_rnn(enc_cell, rnn_inputs, dtype = tf.float32)
