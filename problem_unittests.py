# -*- coding: utf-8 -*-
"""
Created on Sat Jul 20 21:34:15 2019
...
def test_encoding_layer(encoding_layer):
    ...
    with tf.Graph().as_default():
        ...
        bad_types = [type(state) for state in states if not isinstance(state, tf.nn.rnn_cell.LSTMStateTuple)]
...
def test_decoding_layer_train(decoding_layer_train):
    ...
    with tf.Graph().as_default():
        with tf.variable_scope("decoding") as decoding_scope:
            dec_cell = tf.compat.v1.nn.rnn_cell.MultiRNNCell([tf.compat.v1.nn.rnn_cell.LSTMCell(rnn_size)] * num_layers)
